...
export const IntegratedView = ({
    ...
    // Filter communities based on search query, search type and my communities filter
    const filteredCommunities = useMemo(() => {
        // Start with all communities or just mine based on filter
        let filtered = localCommunities;
        
        // Apply my communities filter if enabled
        if (showOnlyMyComm) {
        ...
        return filtered.filter(community => {
            switch (searchType) {
                case 'name':
                    return community.name.toLowerCase().includes(queryLower);
                case 'description':
                    return community.description.toLowerCase().includes(queryLower);
                case 'topics':
                    return community.topics.some(topic => 
                        topic.toLowerCase().includes(queryLower)
                    );
                default: // 'all'
                    return (
                        community.name.toLowerCase().includes(queryLower) ||
                        community.description.toLowerCase().includes(queryLower) ||
    ...
    }, [localCommunities, searchQuery, searchType, showOnlyMyComm]);
    ...
    // Estado para mostrar el diálogo de confirmación para unirse
    const [showJoinConfirmation, setShowJoinConfirmation] = useState<string | null>(null);

    const handleCommunityClick = (community: Community) => {
        // Si el usuario ya es miembro, mostrar los posts
        if (community.isMember || community.isCreator) {
            setShowCommunityList(false);
            setSelectedCommunityId(community.id);
            setSelectedTopic(null); // Reset topic filter
        } else {
            // Si no es miembro, mostrar confirmación para unirse
            setShowJoinConfirmation(community.id);
        }
    };

    // Manejar la confirmación para unirse a una comunidad
    const handleJoinConfirmation = (communityId: string) => {
        handleJoinCommunity(communityId);
        setShowJoinConfirmation(null);
        // No cambiamos inmediatamente a la vista de posts, esperamos a que se una
    };
    ...
    const renderCommunitiesList = () => (
        <div>
            {/* Search Communities Container */}
            <div className="border-2 border-[var(--matrix-green)] rounded-lg p-4 mb-5 bg-black bg-opacity-70 shadow-lg">
                {/* Title and My Communities Toggle */}
                <div className="flex items-center justify-between mb-3 border-b border-[var(--matrix-green)] pb-2">
                    <span className="text-[var(--matrix-green)] text-xl font-bold">Search Communities</span>
                    
                    <div className="flex items-center space-x-2">
                        <span className="text-sm text-gray-300">My Communities</span>
                        <button 
                            onClick={() => setShowOnlyMyComm(!showOnlyMyComm)}
                            className={`relative inline-flex h-6 w-11 items-center rounded-full transition-colors focus:outline-none focus:ring-2 focus:ring-[var(--matrix-green)] focus:ring-offset-2 ${showOnlyMyComm ? 'bg-[var(--matrix-green)]' : 'bg-gray-700'}`}
                        >
                            <span 
                                className={`inline-block h-4 w-4 transform rounded-full bg-white transition ${showOnlyMyComm ? 'translate-x-6' : 'translate-x-1'}`} 
                            />
                        </button>
                    </div>
                </div>
                
                {/* Search Input with Dynamic Icon and Clear Button */}
                <div className="relative mb-4">
                    <div className="absolute inset-y-0 left-0 pl-3 flex items-center pointer-events-none">
                        {searchType === 'all' && <span className="text-[var(--matrix-green)]">🌐</span>}
                        {searchType === 'name' && <span className="text-[var(--matrix-green)]">📝</span>}
                        {searchType === 'description' && <span className="text-[var(--matrix-green)]">📜</span>}
                        {searchType === 'topics' && <span className="text-[var(--matrix-green)]">🎯</span>}
                    </div>
                    <input
                        type="text"
                        value={searchQuery}
                        ...
                        className="bg-black border-2 border-[var(--matrix-green)] text-white p-2 pl-10 pr-10 rounded w-full focus:ring-2 focus:ring-[var(--matrix-green)] focus:outline-none"
                    />
                    {searchQuery && (
                        <button 
                            onClick={() => setSearchQuery('')}
                            className="absolute inset-y-0 right-0 pr-3 flex items-center text-[var(--matrix-green)] hover:text-white transition-colors"
                        >
                ...
                <div className="flex flex-col space-y-2">
                    <p className="text-[var(--matrix-green)] text-sm font-medium ml-1">Search in:</p>
                    <div className="flex flex-wrap gap-2">
                        <button
                            onClick={() => setSearchType('all')}
                            className={`px-3 py-1.5 rounded-md flex items-center text-sm transition-colors duration-200 ${searchType === 'all' 
                                ? 'bg-[var(--matrix-green)] text-black' 
                                : 'bg-black border border-[var(--matrix-green)] text-[var(--matrix-green)] hover:bg-[rgba(0,255,0,0.1)]'}`}
                        >
                            <span className="mr-1">🌐</span> All
                        </button>
                        <button
                            onClick={() => setSearchType('name')}
                            className={`px-3 py-1.5 rounded-md flex items-center text-sm transition-colors duration-200 ${searchType === 'name' 
                                ? 'bg-[var(--matrix-green)] text-black' 
                                : 'bg-black border border-[var(--matrix-green)] text-[var(--matrix-green)] hover:bg-[rgba(0,255,0,0.1)]'}`}
                        >
                        ...
                        <button
                            ...
                            className={`px-3 py-1.5 rounded-md flex items-center text-sm transition-colors duration-200 ${searchType === 'description' 
                                ? 'bg-[var(--matrix-green)] text-black' 
                                : 'bg-black border border-[var(--matrix-green)] text-[var(--matrix-green)] hover:bg-[rgba(0,255,0,0.1)]'}`}
                        >
                        ...
                        <button
                            ...
                            className={`px-3 py-1.5 rounded-md flex items-center text-sm transition-colors duration-200 ${searchType === 'topics' 
                                ? 'bg-[var(--matrix-green)] text-black' 
                                : 'bg-black border border-[var(--matrix-green)] text-[var(--matrix-green)] hover:bg-[rgba(0,255,0,0.1)]'}`}
                        >
            ...
            <div className="space-y-4">
                {filteredCommunities.length === 0 ? (
                    <p className="text-center text-gray-400">No communities found. Try a different search or create a new community!</p>
                ) : (
                    [...filteredCommunities]
                        .sort((a, b) => {
                            ...
                            >
                                ...
                                <div className="flex justify-between items-center">
                                    ...
                                    {/* Solo mostrar botón Leave para miembros no creadores */}
                                    {!community.isCreator && community.isMember && (
                                        <button
                                            ...
                                            className={`px-3 py-1 rounded text-sm font-medium ${leavingCommunityId === community.id
    ...
    // Renderizar el diálogo de confirmación para unirse a una comunidad
    const renderJoinConfirmationDialog = () => {
        if (!showJoinConfirmation) return null;
        
        const community = localCommunities.find(c => c.id === showJoinConfirmation);
        if (!community) return null;
        
        return (
            <div className="fixed inset-0 bg-black bg-opacity-70 flex items-center justify-center z-50">
                <div className="bg-black border-2 border-[var(--matrix-green)] p-6 rounded-lg max-w-md w-full">
                    <h2 className="text-xl font-bold text-[var(--matrix-green)] mb-4">Join Community</h2>
                    <p className="text-white mb-6">Do you want to join the <span className="font-bold text-[var(--matrix-green)]">{community.name}</span> community?</p>
                    <p className="text-gray-300 mb-6 text-sm">You must be a member to view and create posts in this community.</p>
                    ...
                    <div className="flex justify-end space-x-3">
                        ...
                        <button 
                            onClick={() => handleJoinConfirmation(community.id)}
                            className="px-4 py-2 bg-[var(--matrix-green)] text-black rounded hover:bg-opacity-80"
